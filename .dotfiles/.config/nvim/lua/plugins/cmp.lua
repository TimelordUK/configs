return {
   ...
   dependencies = {
      ...
      },
      {
         ...
         event = "InsertEnter",
         config = true,
         -- use opts = {} for passing setup options:w
      ...
      -- s<space><space> leap to start end line ahead
      {
         "ggandor/leap.nvim",
         opts = {},
      ...
      },
      {
         ...
         init = function()
            ...
            wk.add({
               { "<leader>",  group = ".mini" },
               { "<leader>-", "<CMD>lua MiniFiles.open()<CR>", desc = "open mini" },
            })
         end,
      },
      {
            ...
            wk.add(
               {
                  { "g",  group = ".lsp" },
                  -- Jump to declaration
                  { "gD", vim.lsp.buf.declaration,     desc = "jump declaration" },
                  -- Jump to the definition
                  { "gd", vim.lsp.buf.definition,      desc = "jump definition" },
                  -- Lists all the implementations for the symbol under the cursor
                  { "gi", vim.lsp.buf.implementation,  desc = "list implementations" },
                  -- Show diagnostics in a floating window
                  { "gl", vim.diagnostic.open_float,   desc = "diag in float" },
                  -- Jumps to the definition of the type symbol
                  { "go", vim.lsp.buf.type_definition, desc = "jump type symbol" },
            ...
            wk.add(
               {
                  ...
                  -- Format current file
                  { "<F3>", vim.lsp.buf.format,       desc = "reformat" },
                  ...
                  { "]d",   vim.diagnostic.goto_next, desc = "goto diag" },
               })
         end
